...
// IconType is stored by Geometry Dash as a zero-based category index.
// The limits below match Geometry Dash 2.2081's icon key ranges.
constexpr std::array<IconCategory, 8> kCategories {{
    { "cube-ids",   "Cubos",  static_cast<IconType>(0), 485 },
    { "ship-ids",   "Naves",  static_cast<IconType>(1), 169 },
    { "ball-ids",   "Bolas",  static_cast<IconType>(2), 118 },
    { "ufo-ids",    "UFO",    static_cast<IconType>(3), 149 },
    { "wave-ids",   "Wave",   static_cast<IconType>(4), 96  },
    { "robot-ids",  "Robot",  static_cast<IconType>(5), 68  },
    { "spider-ids", "Spider", static_cast<IconType>(6), 69  },
    { "swing-ids",  "Swing",  static_cast<IconType>(7), 43  },
}};
...
void applyConfiguredUnlocks(bool showResult) {
    ...
    if (showResult) {
        auto message = processed > 0
            ...
            : std::string("No se encontraron IDs validos para desbloquear.");
...
$execute {
    ...
    listenForSettingChanges("apply-icon-unlocks", [](bool enabled) {
        if (enabled) {
            applyConfiguredUnlocks(true);
        }
    });
